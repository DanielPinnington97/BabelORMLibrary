...
public abstract class BabelDBManager implements BabelPersistenceConfig {
    ...
    public BabelDBManager() {
        ...
        if(BabelSettings.getINST().PERST == BabelConSettings.DB_TYPE.MYSQL) {
            DBCONFIG = BabelMySQLQueries.getINST();
        }

        if(BabelSettings.getINST().PERST == BabelConSettings.DB_TYPE.POSTGRES) {
            DBCONFIG = BabelPostgresQueries.getINST();
        }

        if(BabelSettings.getINST().PERST == BabelConSettings.DB_TYPE.HSQL) {
